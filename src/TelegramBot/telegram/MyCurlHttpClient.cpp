...
MyCurlHttpClient::MyCurlHttpClient() : _httpParser() {}

MyCurlHttpClient::~MyCurlHttpClient() {
...
static CURL* getCurlHandle(const MyCurlHttpClient* c_) {
    auto* c = const_cast<MyCurlHttpClient*>(c_);
...
std::string MyCurlHttpClient::makeRequest(const Url& url, const std::vector<HttpReqArg>& args) const {
    ...
    // ITS ALL BECAUSE OF THIS FUCKING LINE
    curl_easy_setopt(curl, CURLOPT_TIMEOUT, telegram_bot::config.telegramTimeoutSec);
